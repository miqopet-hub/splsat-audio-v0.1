...
camera.up.set(0,1,0);
let renderer=null,viewer=null,canvas=null,currentScene=SCENES[0],splatReady=false;
...
function buildViewer(){
  ...
  renderer.setPixelRatio(Math.min(devicePixelRatio||1,isMobile()?1.05:1.5));renderer.setSize(innerWidth,innerHeight);renderer.setClearColor(0x000000,1);stage.appendChild(renderer.domElement);
  viewer=new GaussianSplats3D.Viewer({selfDrivenMode:false,renderer,camera,threeScene:new THREE.Scene(),useBuiltInControls:false,sharedMemoryForWorkers:false,gpuAcceleratedSort:false,integerBasedSort:false,sphericalHarmonicsDegree:0,renderMode:GaussianSplats3D.RenderMode.Always,sceneRevealMode:GaussianSplats3D.SceneRevealMode.Gradual,logLevel:GaussianSplats3D.LogLevel.None,freeIntermediateSplatData:true});
  bindCanvasControls();
}
...
async function loadScene(s){
  splatReady=false;sceneSelect.disabled=true;resetViewButton.disabled=true;currentScene=s;status.textContent=`loading ${s.label}…`;buildViewer();
  try{
    ...
    updateBounds();resetHumanView();rebuildSoundWorld();splatReady=true;status.textContent=`${s.label} · 20 spatial sounds`;
  }catch(e){console.error(e);status.textContent=`${s.label} failed to load`}finally{sceneSelect.disabled=false;resetViewButton.disabled=false}
}

function updateBounds(){
  try{const m=viewer?.splatMesh;if(m?.computeBoundingBox){m.computeBoundingBox();if(m.boundingBox&&!m.boundingBox.isEmpty()){bounds.copy(m.boundingBox);return}}}catch(e){console.warn('bounds',e)}
...
function resetHumanView(){
  ...
  camera.position.set(c.x,eye,c.z+Math.max(s.z*.08,.35));camera.lookAt(c.x,eye,c.z-Math.max(s.z*.22,1));
  const d=new THREE.Vector3();camera.getWorldDirection(d);yaw=targetYaw=Math.atan2(-d.x,-d.z);pitch=targetPitch=0;camera.rotation.z=0;wheelVelocity=0;moveTouch={x:0,y:0};if(moveKnob)moveKnob.style.transform='translate(0,0)';
}
...
function distributeZones(){
  ...
  for(let row=0;row<4;row++)for(let col=0;col<5;col++){const i=row*5+col;z.push({...PRESETS[i],position:new THREE.Vector3(c.x+(col/4-.5)*w*.78+Math.sin(i*2.17)*w*.03,y+Math.sin(i)*.12,c.z+(row/3-.5)*d*.78+Math.cos(i*1.73)*d*.03),radius})}
  return z;
}
...
function noise(ctx,c,z){const buf=ctx.createBuffer(1,ctx.sampleRate*2,ctx.sampleRate),data=buf.getChannelData(0);let last=0;for(let i=0;i<data.length;i++){const w=Math.random()*2-1;last=last*z.shade+w*(1-z.shade);data[i]=last*2}const src=ctx.createBufferSource(),g=ctx.createGain();src.buffer=buf;src.loop=true;g.gain.value=.20;src.connect(g).connect(c.input);src.start();return c}
...
function updateAudio(){
  if(!audio)return;const ctx=audio.ctx,now=ctx.currentTime,f=new THREE.Vector3(0,0,-1).applyQuaternion(camera.quaternion).normalize(),u=new THREE.Vector3(0,1,0).applyQuaternion(camera.quaternion).normalize(),L=ctx.listener;
  ...
  audio.sources.forEach((s,i)=>{const dist=camera.position.distanceTo(s.zone.position),n=THREE.MathUtils.clamp(1-dist/s.zone.radius,0,1),r=rank.get(i),rankGain=r<3?1:r<6?.34:.04,prox=Math.pow(n,.78),vol=.0001+prox*.30*rankGain,cut=650+Math.pow(n,.72)*9000;s.gain.gain.setTargetAtTime(vol,now,.11);s.filter.frequency.setTargetAtTime(cut,now,.14)})
}

let previous=performance.now();
function frame(now){
  requestAnimationFrame(frame);const dt=Math.min((now-previous)/1000,.05);previous=now;updateNavigation(dt);updateAudio();if(viewer){viewer.update();viewer.render()}
  if(audio&&splatReady){const nearest=audio.sources.map(s=>({name:s.zone.name,d:camera.position.distanceTo(s.zone.position)})).sort((a,b)=>a.d-b.d).slice(0,3);status.textContent=`${currentScene.label}\nnearest: ${nearest.map(x=>`${x.name} ${x.d.toFixed(1)}m`).join(' · ')}`}
}
requestAnimationFrame(frame);
addEventListener('resize',()=>{camera.aspect=innerWidth/innerHeight;camera.fov=isMobile()?67:62;camera.updateProjectionMatrix();renderer?.setPixelRatio(Math.min(devicePixelRatio||1,isMobile()?1.05:1.5));renderer?.setSize(innerWidth,innerHeight)});
